...
def test_complete_campaign_flow(test_client: TestClient, test_api_key: str, test_db, test_redis):
    ...
    cur = test_db.cursor()
    cur.execute("SELECT status FROM campaigns WHERE task = %s", (campaign_request["task"],))
    result = cur.fetchone()
