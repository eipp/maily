...
# Default values
API_BASE_URL="https://api.justmaily.com"
TEST_DURATION="5m"
VIRTUAL_USERS=50
RAMP_UP_TIME="30s"
RAMP_DOWN_TIME="30s"
  ...
  <script>
    ...
    fetch('LOAD_TEST_RESULTS_JSON')
      ...
      .then(data => {
        ...
        const httpMetrics = [
          { name: 'Total Requests', value: data.metrics.http_reqs.count },
          { name: 'Failed Requests', value: data.metrics.http_req_failed.fails },
          { name: 'Avg Request Duration', value: formatDuration(data.metrics.http_req_duration.avg) },
          { name: 'P95 Request Duration', value: formatDuration(data.metrics.http_req_duration.p(95)) },
        ];
        
        httpMetrics.forEach(metric => {
          const row = httpMetricsTable.insertRow();
