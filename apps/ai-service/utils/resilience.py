"""
Resilience Utility for AI Service

This module provides resilience engineering utilities for the AI service,
...
import asyncio
import functools
from typing import Callable, Any, Dict, Optional, TypeVar, Awaitable, cast
from enum import Enum

logger = logging.getLogger("ai_service.utils.resilience")
...
def rate_limit(
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            if wait:
                # Wait for tokens to become available, up to max_wait
                start_time = time.time()
                while not limiter.acquire(tokens):
                    if time.time() - start_time > max_wait:
                        raise RateLimitExceededError(limiter.name)
                    time.sleep(0.01)  # Small sleep to avoid busy waiting
            elif not limiter.acquire(tokens):
                raise RateLimitExceededError(limiter.name)
    ...
    def decorator(func: AsyncF) -> AsyncF:
        ...
        async def wrapper(*args: Any, **kwargs: Any) -> Any:
            if wait:
                # Wait for tokens to become available, up to max_wait
                start_time = time.time()
                while not await limiter.acquire_async(tokens):
                    if time.time() - start_time > max_wait:
                        raise RateLimitExceededError(limiter.name)
                    await asyncio.sleep(0.01)  # Small sleep to avoid busy waiting
            elif not await limiter.acquire_async(tokens):
                raise RateLimitExceededError(limiter.name)
