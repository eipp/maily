...
/**
 * Blockchain Service Verifier
 * 
 * This script verifies the deployment of the blockchain service
 * by checking its health, connectivity, and functionality.
 * 
 * Usage: ./blockchain-verifier.js <environment> <component> <version>
 */
...
async function verifyKubernetesDeployment(config) {
  ...
  const namespace = blockchainConfig.namespace || 'maily';
  const deployment = blockchainConfig.deployment || 'blockchain';
  ...
  try {
    ...
    if (!imageFound) {
      results.warnings.push(`Container 'blockchain' not found in deployment ${deployment}`);
    }
...
async function verifyHealthEndpoint(config) {
  ...
  try {
    ...
    const response = await axios.get(`https://${blockchainDomain}/health`, {
      timeout: 10000,
    ...
    if (response.status === 200) {
      ...
      if (data.blockchain_connected === true) {
        results.passed.push('Blockchain service is connected to Ethereum network');
      } else {
        results.failed.push('Blockchain service is not connected to Ethereum network');
        return false;
      }
...
/**
 * Verify Ethereum connectivity
 */
async function verifyEthereumConnectivity(config) {
  console.log(`${colors.blue}Verifying Ethereum connectivity...${colors.reset}`);
 ...
 */
async function verifySmartContractDeployment(config) {
  console.log(`${colors.blue}Verifying smart contract deployment...${colors.reset}`);
  ...
  try {
    ...
    const response = await axios.get(`https://${blockchainDomain}/api/v1/contracts`, {
      timeout: 10000
    });
    ...
    if (response.status === 200) {
      ...
      if (contracts.length === 0) {
        results.warnings.push('No deployed contracts found');
      } else {
        results.passed.push(`Found ${contracts.length} deployed contract(s)`);
        
        // Check each contract
        for (const contract of contracts) {
          results.passed.push(`Contract ${contract.name} deployed at ${contract.address}`);
          
          // Verify contract on Etherscan if available
          if (contract.etherscan_url) {
            results.passed.push(`Contract ${contract.name} verified on Etherscan: ${contract.etherscan_url}`);
          }
        }
      }
      ...
      results.warnings.push(`Failed to get deployed contracts: ${response.status}`);
      return false;
    }
  } catch (error) {
    results.warnings.push(`Failed to verify smart contract deployment: ${error.message}`);
    return false;
  }
}

/**
 * Verify blockchain service functionality
 */
async function verifyBlockchainServiceFunctionality(config) {
  console.log(`${colors.blue}Verifying blockchain service functionality...${colors.reset}`);
  ...
  try {
    // Test contract read endpoint
    const response = await axios.get(
      `https://${blockchainDomain}/api/v1/contracts/read`,
      {
        params: {
          contract: 'MailyTrust',
          method: 'getVersion'
    ...
    if (response.status === 200) {
      results.passed.push('Successfully called contract read endpoint');
      
      const data = response.data;
      if (data.result !== undefined) {
        results.passed.push(`Contract read successful: ${JSON.stringify(data.result)}`);
      } else {
        results.warnings.push('Contract read returned undefined result');
      }
      
      return true;
    } else {
      results.warnings.push(`Contract read endpoint returned status ${response.status}`);
      return false;
    }
  } catch (error) {
    results.warnings.push(`Failed to verify blockchain service functionality: ${error.message}`);
    return false;
  }
}

/**
 * Verify blockchain service metrics
 */
async function verifyMetrics(config) {
  console.log(`${colors.blue}Verifying blockchain service metrics...${colors.reset}`);
  ...
  try {
    ...
    const response = await axios.get(`https://${blockchainDomain}/metrics`, {
      timeout: 10000,
      validateStatus: null // Accept any status code
    });
...
function printResults() {
  console.log('\n' + '='.repeat(80));
  console.log(`${colors.blue}Blockchain Service Verification Results${colors.reset}`);
...
async function main() {
  try {
    ...
    // Verify Ethereum connectivity
    const ethereumVerified = await verifyEthereumConnectivity(config);
    if (!ethereumVerified) {
      printResults();
      return;
    }
    ...
    // Verify smart contract deployment
    await verifySmartContractDeployment(config);
    
    // Verify blockchain service functionality
    await verifyBlockchainServiceFunctionality(config);
