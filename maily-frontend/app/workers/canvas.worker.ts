...
function transformImage(imageData: ImageData, type: TransformType, value: number): ImageData {
...
function applyFilter(imageData: ImageData, type: FilterType): ImageData {
  if (!imageData?.data || !imageData.width || !imageData.height) {
    throw new Error('Invalid image data');
  }
