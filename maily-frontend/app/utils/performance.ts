...
export function measurePerformance() {
  ...
  try {
    ...
    if (metrics.loadTime && metrics.loadTime > PERFORMANCE_BENCHMARKS.LOAD_TIME_THRESHOLD) {
      console.warn(`Page load time (${metrics.loadTime}ms) exceeds threshold (${PERFORMANCE_BENCHMARKS.LOAD_TIME_THRESHOLD}ms)`);
    }
  ...
  const countFrames = () => {
    ...
    if (currentTime - lastTime >= 1000) {
      ...
      if (fps < PERFORMANCE_BENCHMARKS.FPS_THRESHOLD) {
        console.warn(`Low FPS detected: ${fps} (threshold: ${PERFORMANCE_BENCHMARKS.FPS_THRESHOLD})`);
      } else {
...
export function usePerformanceMonitoring(componentName: string) {
  ...
  useEffect(() => {
    ...
    if (renderTime > PERFORMANCE_BENCHMARKS.RENDER_TIME_THRESHOLD) {
      console.warn(`${componentName} render time (${renderTime}ms) exceeds threshold (${PERFORMANCE_BENCHMARKS.RENDER_TIME_THRESHOLD}ms)`);
    }
  });
    ...
    measureApiCall: async <T>(apiCall: () => Promise<T>): Promise<T> => {
      ...
      try {
        ...
        if (duration > PERFORMANCE_BENCHMARKS.API_RESPONSE_THRESHOLD) {
          console.warn(`API call exceeded response threshold: ${duration}ms (threshold: ${PERFORMANCE_BENCHMARKS.API_RESPONSE_THRESHOLD}ms)`);
        }
